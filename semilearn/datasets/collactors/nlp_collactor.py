...
class DataCollatorWithPadding:
    ...
    def __call__(self, features):
        ...
        for f in features:
            ...
            if 'text_s' in f and f['text_s'] != 'None':
                # augmented
        ...
        if 'label' in batch:
            if len(s_features)==0:
                ...
                # FIXME: conflicts with other algs
                if is_ulb:
                    # mtl fixmatch
                    return {'idx_ulb': batch['idx'],
                            'x_ulb_w': {'input_ids': batch['input_ids'], 'attention_mask': batch['attention_mask']},
...
# TODO
def get_biobert_collactor(max_length=512):
    tokenizer = BertTokenizerFast.from_pretrained('dmis-lab/biobert-v1.1')
    collact_fn = DataCollatorWithPadding(tokenizer, max_length=max_length)
    return collact_fn

def get_pubmedbert_collactor(max_length=512):
    tokenizer = BertTokenizerFast.from_pretrained('microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract')
    collact_fn = DataCollatorWithPadding(tokenizer, max_length=max_length)
    return collact_fn
