...
def get_collactor(args, net):
    if net == 'bert_base_uncased':
        ...
        collact_fn = get_biobert_collactor(args.max_length)
    elif net == 'pubmedbert':
        from semilearn.datasets.collactors import get_pubmedbert_collactor
        collact_fn = get_pubmedbert_collactor(args.max_length)
    else:
        raise ValueError(f'not implemented: {net}')
    return collact_fn
