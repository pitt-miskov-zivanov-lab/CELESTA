...
class AlgorithmBase:
    ...
    def set_ema_model(self):
        ...
        """
        ema_model = self.net_builder(num_classes=self.num_classes, use_mtl=self.args.use_mtl)
    ...
    def evaluate_mtl(self, eval_dest="eval", out_key="logits", return_logits=False):
        """
        adapted evaluation function with mtl setting
        """
        ...
        eval_loaders = self.loader_dict[eval_dest]

        # TODO: weights of various tasks
        eval_dicts = {}
        for eval_loader in eval_loaders:
            idx = eval_loaders.index(eval_loader)
            task = self.args.tasks_info[idx]
            eval_dict = self._evalute_mtl(eval_loader, task=task,
                              eval_dest=eval_dest, out_key=out_key, return_logits=return_logits)

            eval_dicts[task] = eval_dict

        return eval_dicts

    def _evalute_mtl(self, eval_dataloader, task=None,
                     eval_dest="eval", out_key="logits", return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_dataloader:
                ...
                logits = logits[self.args.tasks_info.index(task)]
